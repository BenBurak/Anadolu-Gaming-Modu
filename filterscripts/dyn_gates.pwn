...
new
	GateStates[2][16] = {"{E74C3C}Kapalý", "{2ECC71}Açýk"};
...
{
    new string[128];
	format(string, sizeof(string), "Gate Durumu\t%s\nGate Þifresi\t%s\nGate Pozisyonu\nGate Açýlacaðý Pozisyon\nGateyi Sil", GateStates[ GateData[id][GateState] ], GateData[id][GatePassword]);
	ShowPlayerDialog(playerid, DIALOG_GATE_EDITMENU, DIALOG_STYLE_TABLIST, "Gate Düzenleme", string, "Seç", "Ýptal");
	return 1;
}
		...
		case EDIT_RESPONSE_FINAL:
		{
		    ...
		    switch(EditingGateType[playerid])
		    {
				case GATE_STATE_CLOSED:
				{
					...
					SendClientMessage(playerid, 0x2ECC71FF, "Bilgi: {FFFFFF}Gatenin varsayýlan pozisyonu ayarlandý.");
					...
					if(GateData[id][GateOpenPos][0] == 0.0 && GateData[id][GateOpenRot][0] == 0.0) {
				        ...
				        SendClientMessage(playerid, 0xF39C12FF, "Uyarý: {FFFFFF}Bu gatenin açýlma pozisyoun yok");
				        SendClientMessage(playerid, 0xF39C12FF, "Uyarý: {FFFFFF}Ýsterseniz gatenin açýlma pozisyonu þimdi veya daha sonra ayarlayabilirsiniz.");
				        SendClientMessage(playerid, 0xF39C12FF, "Uyarý: {FFFFFF}Açýlýþ pozisyonu ayarlanýncaya kadar bu gateyi kullanamazlar.");
				    }else{
				        EditingGateID[playerid] = -1;
		    			EditingGateType[playerid] = -1;
				    }
				...
				{
				    SendClientMessage(playerid, 0x2ECC71FF, "Bilgi: {FFFFFF}Gatenin açýlýþ pozisyonu ayarlandý.");
		...
		case EDIT_RESPONSE_CANCEL:
		{
		    ...
		    switch(EditingGateType[playerid])
		    {
				case GATE_STATE_CLOSED:
				{
					...
					GateData[id][GateRot][1] = ry;
					GateData[id][GateRot][2] = rz;
					SendClientMessage(playerid, 0x2ECC71FF, "Bilgi: {FFFFFF}Gatenin varsayýlan pozisyonu iptal edildi.");
				}

				case GATE_STATE_OPEN:
				{
				    SendClientMessage(playerid, 0x2ECC71FF, "Bilgi: {FFFFFF}Gatenin açýlýþ pozisyonu iptal edildi.");
				    
				    if(GateData[id][GateOpenPos][0] == 0.0 && GateData[id][GateOpenRot][0] == 0.0)
					{
				        SendClientMessage(playerid, 0xF39C12FF, "Uyarý: {FFFFFF}Bu gatenin açýlma pozisyonu yok");
				        SendClientMessage(playerid, 0xF39C12FF, "Uyarý: {FFFFFF}Açýlýþ pozisyonu ayarlanýncaya kadar bu gate kullanýlamaz.");
				    }
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	if(dialogid == DIALOG_GATE_PASSWORD)
	{
		...
		if(!strlen(inputtext)) return ShowPlayerDialog(playerid, DIALOG_GATE_PASSWORD, DIALOG_STYLE_PASSWORD, "Gate Þifresi", "{E74C3C}Ekraný boþ býrakayýn ve gate þifresini girin.", "Aç", "Ýptal");
		new id = GetClosestGate(playerid);
		if(id == -1) return SendClientMessage(playerid, 0xE74C3CFF, "Hata: {FFFFFF}Bir gatenin yakýnýnda deðilsin.");
		if(strcmp(GateData[id][GatePassword], inputtext)) return ShowPlayerDialog(playerid, DIALOG_GATE_PASSWORD, DIALOG_STYLE_PASSWORD, "Gate Þifresi", "{E74C3C}Hatalý þifre girdiniz.\n{FFFFFF}Lütfen gate þifresini girin.", "Aç", "Ýptal");
	...
	if(dialogid == DIALOG_GATE_EDITMENU)
	{
		...
		new id = EditingGateID[playerid];
		if(id == -1) return SendClientMessage(playerid, 0xE74C3CFF, "Hata: {FFFFFF}Bir gate düzenlemiyorsun.");
	    if(listitem == 0)
		{
			ToggleGateState(id);
	    ...
	    if(listitem == 1) ShowPlayerDialog(playerid, DIALOG_GATE_NEWPASSWORD, DIALOG_STYLE_INPUT, "Gate Þifresi Deðiþtir", "Gate þifresini girin.\n\nÞifresiz gate istiyorsanýz ekraný boþ býrakýn.", "Güncelle", "Ýptal");
		if(listitem == 2)
		{
		    SetGateState(id, GATE_STATE_CLOSED, 2);
		    EditingGateType[playerid] = GATE_STATE_CLOSED;
		    EditDynamicObject(playerid, GateData[id][GateObject]);
		    SendClientMessage(playerid, 0x2ECC71FF, "Bilgi: {FFFFFF}Gatenin varsayýlan konumunu belirleyin.");
		}
		...
		if(listitem == 3)
		{
		    ...
		    EditingGateType[playerid] = GATE_STATE_OPEN;
		    EditDynamicObject(playerid, GateData[id][GateObject]);
		    SendClientMessage(playerid, 0x2ECC71FF, "Bilgi: {FFFFFF}Gatenin açýlacaðý pozisyonu belirleyin.");
		}
	...
	if(dialogid == DIALOG_GATE_NEWPASSWORD)
	{
	    if(!IsPlayerAdmin(playerid)) return 1;
		new id = EditingGateID[playerid];
		if(id == -1) return SendClientMessage(playerid, 0xE74C3CFF, "Hata: {FFFFFF}Bir gate düzenlemiyorsunuz.");
	    ...
	    SendClientMessage(playerid, 0x2ECC71FF, "Bilgi: {FFFFFF}Þifre güncellendi.");
	    SaveGate(id);
	    ShowEditMenu(playerid, id);
	    return 1;
	}
...
{
	new id = GetClosestGate(playerid);
	if(id == -1) return SendClientMessage(playerid, 0xE74C3CFF, "Hata: {FFFFFF}Bir gatenin yanýnda deðilsin.");
	if(GateData[id][GateEditing]) return SendClientMessage(playerid, 0xE74C3CFF, "Hata: {FFFFFF}Bu gate düzenlenirken kullanamazsýn.");
	if(GateData[id][GateOpenPos][0] == 0.0 && GateData[id][GateOpenRot][0] == 0.0) return SendClientMessage(playerid, 0xE74C3CFF, "Hata: {FFFFFF}Bu gatenin açýlma pozisyonu yok.");
	if(!strlen(GateData[id][GatePassword])) {
	    ...
	    if(HasGateAuth[playerid][id]) {
		    ...
		    ShowPlayerDialog(playerid, DIALOG_GATE_PASSWORD, DIALOG_STYLE_PASSWORD, "Gate Þifresi", "Açmak istediðiniz gatenin þifresini girin.", "Aç", "Kapat");
		}
	}
	return 1;
}
...
{
	if(!IsPlayerAdmin(playerid)) return SendClientMessage(playerid, 0xE74C3CFF, "Hata: {FFFFFF}Rcon admin olmalýsýn.");
	if(EditingGateID[playerid] != -1) return SendClientMessage(playerid, 0xE74C3CFF, "Hata: {FFFFFF}Þuanda bir gate oluþturuyorsun.");
	new id = Iter_Free(Gates);
	if(id == -1) return SendClientMessage(playerid, 0xE74C3CFF, "Hata: {FFFFFF}Gate oluþturma limiti dolmuþ.");
	new model, password[GATE_PASS_LEN];
	if(sscanf(params, "iS()["#GATE_PASS_LEN"]", model, password)) return SendClientMessage(playerid, 0xF39C12FF, "Kullaným: {FFFFFF}/GateOlustur [model id] [þifre (isteðe baðlý)]");
	GateData[id][GateModel] = model;
	GateData[id][GatePassword] = password;
	...
	EditingGateType[playerid] = GATE_STATE_CLOSED;
	EditDynamicObject(playerid, GateData[id][GateObject]);
	SendClientMessage(playerid, 0x2ECC71FF, "Bilgi: {FFFFFF}Gate oluþturuldu, düzenlemeyi yapýn.");
	return 1;
}
...
CMD:gateduzenlebrkn(playerid, params[])
{
	...
	if(!IsPlayerInRangeOfPoint(playerid, 20.0, GateData[id][GatePos][0], GateData[id][GatePos][1], GateData[id][GatePos][2])) return SendClientMessage(playerid, 0xE74C3CFF, "Hata: {FFFFFF}Düzenlemek istediðiniz gatenin yakýnýnda olmalýsýnýz.");
	GateData[id][GateEditing] = true;
	EditingGateID[playerid] = id;
	ShowEditMenu(playerid, id);
	return 1;
